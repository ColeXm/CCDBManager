...
static NSString *const UPDATE_ITEM_SQL = @"REPLACE INTO %@ (id, json, createdTime, modelName) values (?, ?, ? ,?)";

static NSString *const QUERY_ITEM_SQL = @"SELECT json, createdTime , modelName from %@ where id = ? Limit 1";
...
- (void)createTable:(NSString *)tableName {
    if ([CCDBManager checkTableName:tableName] == NO) {
        return;
    }
    
    if ([self isTableExists:tableName]) {
        debugLog(@"ERROR, table:%@ already exist", tableName);
        return;
    }
...
- (BOOL)setObject:(id)anObject forKey:(NSString*)key table:(NSString *)tableName{
    ...
    if (!anObject || !key) {
        debugLog(@"ERROR, object or key is nil");
        return NO;
    }
    
    NSString * modelName = @"";
    
    if (![anObject isKindOfClass:[NSArray class]] && ![anObject isKindOfClass:[NSDictionary class]]) {
        modelName =  NSStringFromClass([anObject class]);
        anObject = [anObject propertyDictionary];
    }
    
    NSError * error;
    NSData * data = [NSJSONSerialization dataWithJSONObject:anObject options:0 error:&error];
    if (error) {
    ...
    __block BOOL result;
    [_dbQueue inDatabase:^(FMDatabase *db) {
        result = [db executeUpdate:sql, key, jsonString, createdTime,modelName];
    }];
    if (!result) {
...
- (id)objectForKey:(NSString *)key table:(NSString *)tableName{
    CCDBItem * item = [self itemForKey:key table:tableName];
    if (item) {
...
- (CCDBItem *)itemForKey:(NSString *)key table:(NSString *)tableName{
    if ([CCDBManager checkTableName:tableName] == NO) {
        return nil;
    }
    ...
    [_dbQueue inDatabase:^(FMDatabase *db) {
        ...
        if ([rs next]) {
            ...
            modelName = [rs stringForColumn:@"modelName"];
        }
        [rs close];
    }];
    if (json) {
        ...
        if (modelName.length > 0) {
            result = [NSClassFromString(modelName) objectWithDictionary:result];
        }
        
        CCDBItem * item = [[CCDBItem alloc] init];
        item.itemId = key;
